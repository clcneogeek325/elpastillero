...
#def aplicar_descuento(id_personal):
#	lista_productos = Producto.objects.filter(personal_id=id_personal)
#	personal = Personal.objects.get(user_id=id_personal)
#	for p in lista_productos:
#		if p.producto.categoria == personal.pc.categoria:
#			p.ve
...
def view_cancel_venta(request):
	if request.user.is_authenticated():
		personal = obt_personal(request,request.user.id)
		if request.method == "POST":
			print "Fue un POST"
			form = ventasForm()
			lista_productos = Tabla_temporal.objects.filter(personal_id=personal.id)
