function tipo_compania(){
	...
	$("#id_descuento_compra").keyup(function(evento){
		...
		$("#id_precio_compra").attr("value",parseInt(precio_farmacia-descuento));
	});
	...
	$("#id_descuento_venta").keyup(function(evento){
		...
		$("#id_precio_venta").attr("value",parseInt(precio_sugerido-porcentage));
		var precio_venta = $("#id_precio_venta").attr("value");
		var precio_compra = $("#id_precio_compra").attr("value");
		var precio_farmacia = $("#id_precio_farmacia").attr("value");
		$("#id_ganancia").attr("value",(precio_venta-precio_compra).toFixed(2));
	...
	if(prov == "11" || prov == "12"){
			...
			if(parseInt($("#id_descuento_compra").attr("value")) == 0)
					{
					var precio_farmacia = $("#id_precio_farmacia").attr("value");
					$("#id_precio_compra").attr("value",parseInt(precio_farmacia));
		
					}else{
					var precio_sugerido=$("#id_precio_sugerido").attr("value");
					var descuento_compra=$("#id_descuento_compra").attr("value");
					var descuento=(precio_sugerido/100)*descuento_compra;
					$("#id_precio_compra").attr("value",(precio_sugerido-descuento).toFixed(2));
					}
	...
	if(prov == "13" || prov == "14" || prov == "25" || prov == "10"){
					$("#id_descuento_compra").keyup(function(evento){
						var precio_sugerido=$("#id_precio_sugerido").attr("value");
						var descuento_compra=$("#id_descuento_compra").attr("value");
						var descuento=(precio_sugerido/100)*descuento_compra;
