function tipo_compania(){
	...
	$("#id_descuento_compra").keyup(function(evento){
		...
		var descuento=(precio_farmacia/100.00)*descuento_compra;
	...
	$("#id_descuento_venta").keyup(function(evento){
		...
		var porcentage=(precio_sugerido/100.00)*descuento_venta;
		$("#id_precio_venta").attr("value",precio_sugerido);
		$("#id_precio_venta").attr("value",(parseFloat(precio_sugerido)-parseFloat(porcentage)).toFixed(2));
		var precio_venta = $("#id_precio_venta").attr("value");
		var precio_compra = $("#id_precio_compra").attr("value");
		var precio_farmacia = $("#id_precio_farmacia").attr("value");
		$("#id_ganancia").attr("value",parseFloat(precio_venta)-parseInt(precio_compra));
	...
	if(prov == "11" || prov == "12"){
			...
			if(parseFloat$("#id_descuento_compra").attr("value")) == 0)
					{
					var precio_farmacia = $("#id_precio_farmacia").attr("value");
		...
		$("#id_descuento_venta").keyup(function(evento){
			var precio_sugerido=$("#id_precio_sugerido").attr("value");
			var descuento_venta=$("#id_descuento_venta").attr("value");
			var set_porcentege=(precio_sugerido/100.00)*descuento_venta;
			$("#id_precio_venta").attr("value",(parseFloat(precio_sugerido)-parseFloat(set_porcentege)).toFixed(2));
			var precio_venta=$("#id_precio_venta").attr("value");
			var precio_compra=$("#id_precio_compra").attr("value");
			$("#id_ganancia").attr("value",(parseFloat(precio_venta))-(parseFloat(precio_compra)));
		});
	}
